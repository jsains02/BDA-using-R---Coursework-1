...
head(Boston[order(Boston$medv),],1)
```
...
(b) Using the rnorm() function, create a vector, ????????????, containing 100 observations drawn from a N (0, 0. 25) distribution i.e. a normal distribution with mean 0 and variance 0. 25.

```{r}
eps = rnorm(100,0,0.5)
```

(c) Using ???? and ????????????, generate a vector ???? according to the model ???? = ???1 + 0.5???? + ??. What is the length of the vector ????? What are the values of ????! and ????! in this linear model?
